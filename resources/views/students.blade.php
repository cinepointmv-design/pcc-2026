            ...
            <div class="container">
                <div class="row">
                    <div class="col-md-12 row align-items-center heading-wrapper mb-4">
                        ...
                        <div class="col-md-6 col-12">
                            <a href="{{url('/add-students')}}" class="btn d-block ms-md-auto ms-none">Add Student</a>
                        </div>
                    </div>
                    ...
                    <div class="col-md-12 mb-3">
                        <ul class="nav nav-pills" style="gap: 10px;">
                            {{-- Active Students Tab --}}
                            <li class="nav-item">
                                <a class="nav-link {{ !request('status') ? 'active bg-primary text-white' : 'bg-light text-dark border' }}" 
                                   href="{{ url('/students') }}">
                                    Active 
                                    <span class="badge {{ !request('status') ? 'bg-white text-primary' : 'bg-secondary text-white' }} ml-1">
                                        {{ $activeCount ?? 0 }}
                                    </span>
                                </a>
                            </li>
                            ...
                            <li class="nav-item">
                                <a class="nav-link {{ request('status') == 'completed' ? 'active bg-primary text-white' : 'bg-light text-dark border' }}" 
                                   href="{{ url('/students?status=completed') }}">
                                    Completed 
                                    <span class="badge {{ request('status') == 'completed' ? 'bg-white text-primary' : 'bg-secondary text-white' }} ml-1">
                                        {{ $completedCount ?? 0 }}
                                    </span>
                                </a>
                            </li>
                            ...
                            <li class="nav-item">
                                <a class="nav-link {{ request('status') == 'removed' ? 'active bg-primary text-white' : 'bg-light text-dark border' }}" 
                                   href="{{ url('/students?status=removed') }}">
                                    Removed 
                                    <span class="badge {{ request('status') == 'removed' ? 'bg-white text-primary' : 'bg-secondary text-white' }} ml-1">
                                        {{ $removedCount ?? 0 }}
                                    </span>
                                </a>
                            </li>
                                ...
                                <table class="table table-hover data-table">
                                    ...
                                    <tbody id="clientTableBody">
                                        ...
                                        <tr class="client-row">
                                            ...
                                            <td>{{ $student->name }} </td>
                                            {{-- <td>{{ $student->email }} </td> --}}
                                            <td><a href="https://wa.me/+91{{ $student->phone }}">{{ $student->phone }} </a></td>
                                            ...
                                            <td>
                                                @foreach($student->studentCourses as $course)
                                                    {{ $course->course->course_name ?? 'N/A' }}<br>
                                                @endforeach
                                            </td>
                                            ...
                                            <td>
                                                @foreach($student->studentCourses as $course)
                                                    @php
                                                        if(isset($course->course->duration)){
                                                            $totalDuration += $course->course->duration; 
                                                        }
                                                    @endphp
                                                @endforeach

                                                @php
                                                    $joinDate = $student->joiningdate ? \Carbon\Carbon::parse($student->joiningdate) : now();
                                                    $endDate = $joinDate->copy()->addMonths($totalDuration);
                                                    $courseTimeOver = $endDate->isPast();
                                                @endphp
                                                ...
                                                @else
                                                    @if($courseTimeOver)
                                                        <span class="text-warning font-weight-bold">Time Over</span>
                                                    @else
                                                        <span class="text-primary font-weight-bold">Ongoing</span>
                                                    @endif
                                                @endif
                                            </td>
                                            {{-- <td>
                                            ...
                                            <td>
                                                ...
                                                <div class="d-flex align-items-center">
                                                    <a class="edit-btn" href="{{url('/view-students/' . $student->id )}}">View</a>
                                                </div>
                                            </td>
                                            {{-- <td>
                    ...
                    success: function (response) {
                        if (response && response.students && response.students.length > 0) {
                            ...
                            response.students.forEach(student => {
                                ...
                                } else if (student.status === 'Removed') {
                                    statusHtml = '<span class="badge bg-danger">Removed</span>';
                                } else {
                                    statusHtml = '<span class="text-primary font-weight-bold">Ongoing</span>';
                                }

                                // Buttons Logic - Only View Button Here
                                let actionHtml = `<a class="edit-btn" href="{{url('/view-students') }}/${student.id}">View</a>`;
                            ...
                            $('#clientTableBody').html('<tr><td colspan="8" class="text-center py-4">No students found</td></tr>');
                            $('.table-pagination').html('');
                        }
                    },
